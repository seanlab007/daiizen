...
export default function OrderDetail() {
  ...
  const { data: order, isLoading } = trpc.orders.byId.useQuery({ id: orderId }, { enabled: isAuthenticated && !isNaN(orderId) });
  ...
  if (!order) {
    return (
      <div className="min-h-screen py-8 container text-center py-20">
        ...
        <Button asChild variant="outline" className="mt-4"><Link href="/orders"><ArrowLeft className="w-4 h-4 mr-2" />Back to Orders</Link></Button>
      </div>
    );
  }
    ...
    <div className="min-h-screen py-8">
      <div className="container max-w-2xl">
        <div className="flex items-center gap-3 mb-6">
          <Button variant="ghost" size="sm" asChild><Link href="/orders"><ArrowLeft className="w-4 h-4" /></Link></Button>
          ...
          <div className="divide-y divide-border/50">
            {order.items?.map((item) => (
              <div key={item.id} className="flex items-center gap-3 p-4">
                ...
                <p className="text-sm font-semibold text-primary shrink-0">{Number(item.subtotalUsdd).toFixed(2)} USDD</p>
              </div>
            ))}
          </div>
